...
namespace DataLab.Data.Planning
{
    ...
    public class Plan: INotifyPropertyChanged
    {
        ...
        [XmlArray("CijferLijsten"), XmlArrayItem(typeof(CijferLijst), ElementName = "CijferLijst")]
        public ObservableCollection<CijferLijst> CijferLijsten { get; set; } = new ObservableCollection<CijferLijst>();
        ...
        public void clear(Clear_Options options)
        {
            ...
            ToastNotificationManager.History.Remove("plenner");
        }
        ...
        public void cleanOldItems(TimeSpan saveLimit)
        {

            Activities.Where(act => act.Start < DateTime.Now.Subtract(saveLimit)).ToList().ForEach(act => Activities.Remove(act));

            ToDoItems.Where(tdi => tdi.hasDeadline()).Where(tdi => tdi.Deadline < DateTime.Now.Subtract(saveLimit)).ToList().ForEach(tdi => ToDoItems.Remove(tdi));
        }
